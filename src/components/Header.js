...
// Même catégories utilisées ailleurs sur le site
...
function CategoryMegaMenu() {
  ...
  const { data: activeProductsRaw } = useQuery({
    ...
    queryFn: async () => {
      const res = await client.get(
        `/categories/${activeSlug}/products?limit=6`
      );
  ...
  const products = Array.isArray(activeProductsRaw)
    ? activeProductsRaw
    : activeProductsRaw?.data || [];
  ...
  const activeLabel = CATEGORY_LINKS.find(
    (c) => c.slug === activeSlug
  )?.label;
            ...
            <div className="flex-1 p-5">
              ...
              {products.length > 0 ? (
                ...
                <p className="mt-4 text-xs text-slate-400">
                  Aucun produit à afficher pour le
                  moment.
                </p>
              )}
...
const Header = () => {
  ...
  /*
   * IMPORTANT :
   * On récupère cartCount depuis CartContext.
   * Cela corrige l'erreur :
   * 'cartCount' is not defined
   */
  ...
  useEffect(() => {
    const loadUser = () => {
      const stored =
        localStorage.getItem('dangoUser');
      ...
      window.setTimeout(
        async () => {
          try {
            const response =
              await client.get(
                `/products?limit=10&search=${encodeURIComponent(
                  trimmed
                )}`
              );
